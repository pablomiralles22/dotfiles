...
local not_math_mode = require("snippets.markdown.utils").not_math_mode
local latex_mode = require("snippets.markdown.utils").latex_mode
local md_mode = require("snippets.markdown.utils").md_mode
...
M = {
	...
	{ condition = math_mode, show_condition = math_mode }),
    ...
    autosnippet(
            { trig='mk', name='enter math mode', dscr='enter inline math mode'},
            fmta([[$<>$<>]], { i(1), i(0) }),
            { condition = should_expand_legacy_math_delimeters, show_condition = should_expand_legacy_math_delimeters }
    ),
    autosnippet(
            { trig='mk', name='enter math mode', dscr='enter inline math mode'},
            fmta([[\(<>\)<>]], { i(1), i(0) }),
            { condition = should_expand_modern_math_delimeters, show_condition = should_expand_modern_math_delimeters }
    ),
    autosnippet(
            { trig='dm', name='enter math display mode', dscr='enter display math mode'},
            fmta("$$\n<>\n$$<>", { i(1), i(0) }),
            { condition = should_expand_legacy_math_delimeters, show_condition = should_expand_legacy_math_delimeters }
    ),
}
